...
Explain Caburé in 3 sentences: what it is (Groth16 Phase 2 trusted setup toolkit), what it replaces (p0tion + DefinitelySetup), and the three packages (`@wonderland/cabure-crypto`, `create-cabure-ceremony`, `@cabure/cli`).
...
- **crypto/** — imports from `@wonderland/cabure-crypto` (published package maintained by Lumi)
...
- Package names: `@wonderland/cabure-crypto`, `create-cabure-ceremony`, `@cabure/cli` — never use `elixir-wizard`
...
Ask the developer:
1. Have you done the handoff walkthrough with Lumi on `@wonderland/cabure-crypto`?
