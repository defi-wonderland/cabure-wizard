...
2. **Configure artifacts** - If a circuit path was provided, `.r1cs` files are copied automatically. Otherwise, copy `.r1cs` files into `circuits/` and update `ceremony.config.ts`
3. **Deploy** - Import the generated app into Vercel (or deploy manually)
4. **Contribute** - Contributors visit the frontend or use `@cabure/cli`. Each contribution: download current zkey, collect entropy (mouse/click required in browser), compute in Web Worker or CLI, upload result
